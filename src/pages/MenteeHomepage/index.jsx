...
const HomepagementeeOnePage = () => {
  ...
  const fetchQuestions = () => {
    secured.get("/questions/148d7f85-39b4-482d-aa07-8c20eaefd64d").then((response) => {
  ...
  return (
    <>
      <div className="bg-white_A700 flex flex-col font-nunitosans items-center justify-start mx-auto pb-[1rem] w-full responsive-view">
        ...
        {categoryList && categoryList.length && <HomePageMenteeOneCard3 onChange={reloadQuestions} categoryList={categoryList} className="responsive-questionaire md:px-5 relative w-full" />}

        {questionsList && questionsList.length > 0 ? (
          <>
            {questionsList.map((item, index) => (
              <div key={item._id} className="bg-white_A700 flex flex-col items-start justify-start mt-5 mx-auto p-5 md:px-5 rounded-[30px] shadow-bs2" style={{width: "calc(97vw - 1rem)"}}>
